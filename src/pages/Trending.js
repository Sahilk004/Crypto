...
function Trending() {
  ...
  return (
    <section className='w-[80%] h-full flex flex-col mt-16 mb-24 relative'>
      <div className='flex flex-wrap justify-evenly min-h-[60vh] w-full py-8 border border-gray-100 rounded'>
        {
          trendData && trendData.map(coin =>
             <TrendingCoin key = {data.coin_id} data ={coin.item}/>)
        }
        <button className="w-[2rem] ml-4 hover:scale-110 transition-all transition-ease absolute right-0 -top-10"
