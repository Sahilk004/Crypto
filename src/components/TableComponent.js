...
const SavedBtn = ({data}) => {
  ...
  return(
    ...
    >
                      ...
                      <svg
                        className={`w-[1.5rem] ml-1.5 fill-gray-0 ${allCoins.includes(data.id)?"fill-cyan":"fill-gray-100"} hover:fill-cyan`}
                        width="30"
                        height="30"
                        viewBox="0 0 30 30"
                        fill="grey"
...
function TableComponent() {
  ...
  return (
            ...
            {cryptoData.map((data) => {
              return (
                ...
                >
                  ...
                  <td className={
                    data.market_cap_change_percentage_24h>0?"text-green py-4"
                    :"text-red py-4"
                  }
                  >
                    {Number(
    ...
    <div className="flex mt-4 capitalize flex items-center justify-between">  
        <span>Data provided by <a className="text-cyan" href="https://www.coingecko.com/" target={"_blank"}>CoinGecko</a></span>    
        <Pagination/>
      </div>
    </>
